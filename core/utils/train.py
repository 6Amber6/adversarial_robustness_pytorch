...
class Trainer(object):
    ...
    def standard_loss(self, x, y):
        ...
        out = self.model(x)
        if y.dim() == 2:  # soft labels (e.g. from CutMix)
        ...
        preds = out.detach()
        batch_metrics = {'loss': loss.item(), 'clean_acc': accuracy(y_acc, preds)}
    ...
    def adversarial_loss(self, x, y):
        ...
        """
        y_for_attack = y.argmax(dim=1) if y.dim() == 2 else y
        with ctx_noparamgrad_and_eval(self.model):
            x_adv, _ = self.attack.perturb(x, y_for_attack)
        ...
        if y_adv.dim() == 2:  # soft labels (e.g. from CutMix)
            loss = -(y_adv * F.log_softmax(out, dim=1)).sum(dim=1).mean()
            y_acc = y_adv.argmax(dim=1)
        else:
            loss = self.criterion(out, y_adv)
            y_acc = y_adv
        if self.params.keep_clean:
            y_acc = y.argmax(dim=1) if y.dim() == 2 else y
        
        preds = out.detach()
        batch_metrics = {'loss': loss.item()}
        if self.params.keep_clean:
            preds_clean, preds_adv = preds[:len(x)], preds[len(x):]
            batch_metrics.update({'clean_acc': accuracy(y_acc, preds_clean), 'adversarial_acc': accuracy(y_acc, preds_adv)})
        else:
            batch_metrics.update({'adversarial_acc': accuracy(y_acc, preds)})    
        return loss, batch_metrics
